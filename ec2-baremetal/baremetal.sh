...
master() {
mc1=`mkdir simplek8s`
mc2=`cd ./simplek8s;git init; git pull https://github.com/rangapv/Simplek8s.git`
...
worker() {
wc1=`mkdir simplek8s`
wc2=`cd ./simplek8s;git init; git pull https://github.com/rangapv/Simplek8s.git`
...
if [[ (( $tag1s -eq 0 )) ]]
then
	echo "This is the Master, Prepare git repo simplek8s and nothing more"
        master
 
elif [[ (( $tag2s -eq 0 )) ]]
then
	echo "This is the Worker, Prepare git repo simplek8s and nothing more"
        worker        
